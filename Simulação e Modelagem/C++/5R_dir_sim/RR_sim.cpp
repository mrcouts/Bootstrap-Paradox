...
int main(){
    ...
    vec sigma_ = ((vec){0.3, 0.15, 0.2, 0.15, 0.5, 0.15}) % sign(randn(6));
    ...
    */
    ...
    double eta = 93.5768;
    mat K_; K_.zeros(2,2);
    K_ << 2.6206e+02 << 1.9077e+02 << endr
       << 0.0        << 1.7379e+02 << endr;
    vec k_ = {6.8903, 7.4243};
    
    //SMCLaw SMC = SMCLaw(2, lambda, eta, K_, k_, 100.0, &RefObj, &Robot);
    SMCLaw SMC = SMCLaw(2, lambda, eta, K_, k_, 20.0, &r_, &dr_, &d2r_, &Robot);
