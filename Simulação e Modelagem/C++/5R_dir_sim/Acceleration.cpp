...
Acceleration::Acceleration(int dof, _5R_ *R2, FLControlLaw *u){
	this->dof = dof;
	this->R2 = R2;
	this->u = u;
	caso = 2; }
...
field<vec> Acceleration::Doit(double t, vec q0_, vec q1_){
	...
	switch(caso){
		...
		case 2:
		    ...
		    F(1) = u->Doit(t, q0_ , q1_);
		    double lambda = 10.0;
		    F(0) = solve(join_vert( R2->C_.t()*R2->M_, R2->A_ ), join_vert( R2->Z_.t()*F(1) -R2->C_.t()*(R2->v_ + R2->g_), R2->b_ - 2*lambda*R2->A_*q1_ - lambda*lambda*R2->_q_ ));
		    break; }
	return F; }
