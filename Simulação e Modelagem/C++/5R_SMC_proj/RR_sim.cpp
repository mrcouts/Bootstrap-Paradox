...
int main(){
    ...
    //cube lg__; lg__.zeros(2, 1, nCases);
    //cube m__; m__.zeros(2, 1, nCases);
    //cube Jz__; Jz__.zeros(2, 1, nCases);
                ...
                for(int i4=-1; i4<2; i4 = i4 + 2){
                    for(int i5=-1; i5<2; i5 = i5 + 2){
                        for(int i6=-1; i6<2; i6 = i6 + 2){
                            I__(1,1,0) = Jz1*(1+sigma*i5);
                            I__(2,2,0) = Jz1*(1+sigma*i5);
                            I__(1,1,1) = Jz2*(1+sigma*i6);
                            I__(2,2,1) = Jz2*(1+sigma*i6);
                            P_[cont] = new Mecanismo(2);
                            RR1_[cont] = new Serial(2, {0.12, 0.15}, {lg1*(1+sigma*i1), lg2*(1+sigma*i2)}, {m1*(1+sigma*i3), m2*(1+sigma*i4)}, I__ , {0, -9.8,0}, &fDH_RR);
                            RR2_[cont] = new Serial(2, {0.12, 0.15}, {lg1*(1+sigma*i1), lg2*(1+sigma*i2)}, {m1*(1+sigma*i3), m2*(1+sigma*i4)}, I__ , {0, -9.8,0}, &fDH_RR);
    ...
    mat DeltaA_; DeltaA_.zeros(2,2);
    vec delta_; delta_.zeros(2);
    vec deltaA_; deltaA_.zeros(2);
        ...
        for(uint j=0; j<cols; j++){
            ...
            if(gnr2.convergiu){
                ...
                if(abs(det(Robot.Ao_)) < 1.6*1e-4 || abs(det(A2_)) < 1e-3 ) M(i,j) = 2;
                    ...
                    M(i,j) = 1;
                    Robot.Doit(Robot.q0_, Robot.q1_);
                    fMh_(i,j) = Robot.dy->Mh_;
                    for (uint k = 0; k < nCases; k++){
                        Robot_[k]->Doit(Robot.q0_, Robot.q1_);
                        DeltaA_ = solve(Robot_[k]->dy->Mh_, Robot.dy->Mh_) - eye(2,2);
                        deltaA_ = solve(Robot_[k]->dy->Mh_, Robot.dy->gh_ - Robot_[k]->dy->gh_);
                        Delta_ = arma::max(Delta_, abs(DeltaA_));
                        delta_ = arma::max(delta_, abs(deltaA_));
                    }
                }
