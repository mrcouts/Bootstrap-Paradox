...
{Relatório sobre observadores de ordem reduzida apresentado à disciplina "Introdução à Teoria de Estimação" PME-5009 do Programa de Pós Graduação em Engenharia Mecânica (PPGEM) da Escola Politécnica 
...
iremos obter o modelo linearizado em torno de uma trajetória de referência $\mq_d(t)$  representa-lo no espaço de estados linear.
...
\begin{equation}
\begin{cases}
\dot{\mx} = \mA(t) \cdot \mx + \mB(t) \cdot \mu \\
\my = \mC(t) \cdot \mx
\end{cases}
\end{equation}
...
\begin{figure}[H]
	...
	\caption{Erro de controle $\Delta\theta$}
	\label{fig:1}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Erro de controle $\Delta d$}
	\label{fig:2}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{$\Delta\dot{\theta}$ real (em beje) e estimado (em azul)}
	\label{fig:3}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{$\Delta\dot{d}$ real (em beje) e estimado (em azul)}
	\label{fig:4}
\end{figure}
