...
aplicação do algoritmo de modelagem e simulação para dos mecanismos 5R \cite{22orsino} e 2\underline{R}SU + \underline{P}PaP \cite{Kumazawa}, o projeto de controladores não lineares robustos de alto desempenho baseado no modelo dinâmico para os mecanismos citados, e a simulação e validação experimental das leis de controle sintetizadas.
...
$$ \Rightarrow  \ddot{\mq}\cir = \mA\cir(\mq)^{-1}(\mb(\mq, \dot{\mq}) - \mA\ssh(\mq) \cdot \ddot{\mq}\ssh) $$
\begin{equation} \label{eq:d2q_em_funcao}
\therefore \ddot{\mq} =
\begin{bmatrix}
\mzr \\
\mA\cir(\mq)^{-1} \cdot \mb(\mq, \dot{\mq})
\end{bmatrix}
+
\begin{bmatrix}
\mone \\
\mA\cir(\mq)^{-1} \cdot \mA\ssh(\mq)
\end{bmatrix}
\cdot
\ddot{\mq}\ssh
\end{equation}
...
Sendo: 
\begin{equation}
\mM_{\ssM} (\mq) = (\mZ(\mq)^\msT)^{-1} \cdot \mC(\mq)^\msT \mM'(\mq) \mC(\mq) 
\end{equation}
\begin{equation}
\mv_{\ssM} (\mq, \dot{\mq}\ssh) = (\mZ(\mq)^\msT)^{-1} \cdot \mC(\mq)^\msT \{   \mM'(\mq) \cdot  \mc(\mq, \mC(\mq) \cdot \dot{\mq}\ssh) + \mv'(\mq,\mC(\mq) \cdot \dot{\mq}\ssh) \} 
\end{equation}
\begin{equation}
\mg_{\ssM} (\mq) = (\mZ(\mq)^\msT)^{-1} \cdot \mC(\mq)^\msT \mg'(\mq) 
\end{equation}
...
\ms^\msT (\ddot{\mq}\ssh -\msigma + \varkappa \, \sign(\ms) ) \leq 0
\end{equation} \\
...
(\mone - |\mDelta|_{max} ) \cdot \diag(\underline{k})  \geq |\mdelta|_{max} + |\mDelta|_{max} |\msigma| + \diag(\varkappa \mone)
\end{equation}
...
\begin{equation}
\diag(\underline{k})  = (\mone - |\mDelta|_{max} )^{-1} ( |\mdelta|_{max} + |\mDelta|_{max} |\msigma| + \diag(\varkappa \mone) )
\end{equation}

A qual é válida apenas se $(\mone - |\mDelta|_{max} )^{-1}$ for positiva. 
